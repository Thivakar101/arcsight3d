...
def process_blueprint(image_path):
    ...
    if lines is not None:
        ...
        filtered_lines = []
        min_distance = 200 # Minimum distance between lines to be considered unique
        for i, ((x1, y1), (x2, y2)) in enumerate(line_coordinates):
            ...
            for ((fx1, fy1), (fx2, fy2)) in filtered_lines:
                ...
                if abs((y2 - y1)*(fx2 - fx1) - (fy2 - fy1)*(x2 - x1)) < 1e-2:  # Parallel lines check
                    # Check if both start and end points are within min_distance of each other
